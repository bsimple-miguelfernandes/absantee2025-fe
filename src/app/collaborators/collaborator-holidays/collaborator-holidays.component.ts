...
export class CollaboratorHolidaysComponent {
  ...
  constructor(private route: ActivatedRoute) {
    this.collabId = this.route.snapshot.paramMap.get("collabId")!;
  }

  ngOnInit() {
    this.route.data.subscribe(data => {
      const holidays = data['HolidaysData'];
    ...
    if(this.holidaysForm.length > 0) {
      ...
      if (holidayGroup.valid) {
        if (holidayGroup.value.buttonText == ButtonType.Save) {
          this.collaboratorDataService.addHoliday(this.collabId,
          ...
          this.collaboratorDataService.editHoliday(this.collabId, updatedHoliday).subscribe(h => console.log(h));
